# main.py - Complete Discord Client with Trade-First, Alert-Last Implementation
import os
import sys
import asyncio
import discord
...
class EnhancedDiscordClient(discord.Client):
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            # 🔥 CRITICAL FIX: Use the correct class names
            ...
            # Initialize FAST trade executor with proper event loop reference
            self.trade_executor = FastTradeExecutor(
                self.live_trader,
                self.sim_trader,
            ...
            logger.info("Fast Trade Executor initialized")
...
**Recent Trades:** {len(recent_trades)}
    ...
    async def on_message(self, message):
        """🚀 TRADE-FIRST Enhanced message handling with optimized trade execution"""
        try:
            # Handle commands
            ...
            # Handle trading messages with TRADE-FIRST approach
            ...
            if handler:
                ...
                if raw_msg:
                    # 🚀 TRADE FIRST - Execute trade with minimum latency
                    received_ts = datetime.now(timezone.utc)

                    # Use the new fast trade execution method
                    await self.trade_executor.process_trade_fast(
                        handler, message_meta, raw_msg, SIM_MODE, received_ts,
                        str(message.id), False, self.loop
                    )

                    # 📨 ALERT LAST - Fire and forget live feed (non-blocking)
    ...
    async def on_message_edit(self, before, after):
        """Handle message edits with trade-first approach"""
        try:
...
**Session Age:** {(datetime.now(timezone.utc) - self.start_time).total_seconds() / 3600:.1f}h
    ...
    async def _handle_alert_health_command(self):
        ...
        color = 0x00ff00 if health_data.get(
            'status') == 'HEALTHY' else 0xff8800 if health_data.get('status') == 'WARNING' else 0xff0000
...
**🚀 Enhanced Features:**
- TRADE-FIRST, ALERT-LAST execution (2-4s faster)
- Enhanced tick size detection from Robinhood API
- Minimum tick premium enforcement on all orders
...
- Comprehensive logging to debug.log & errors.log
    ...
    async def _handle_get_price(self, query: str):
        ...
        def blocking_parse_and_fetch():
            ...
            if not all([ticker, strike, opt_type, expiration]):
                missing = [k for k, v in
                           {'ticker': ticker, 'strike': strike, 'type': opt_type, 'expiration': expiration}.items() if
                           not v]
        ...
        if "error" in result:
            await self.alert_manager.add_alert(COMMANDS_WEBHOOK, {"content": f"❌ {result['error']}"},
                                               "command_response")
        else:
            data = result['data']
    ...
    async def _handle_positions_command(self):
        ...
        await self.alert_manager.add_alert(COMMANDS_WEBHOOK, {"content": "⏳ Fetching live account positions..."},
                                           "command_response")
            ...
            try:
                ...
                for p in positions:
                    try:
                        ...
                        if instrument_data:
                            holdings.append(
                                f"• {p['chain_symbol']} {instrument_data['expiration_date']} {instrument_data['strike_price']}{instrument_data['type'].upper()[0]} x{int(float(p['quantity']))}")
                    except Exception as e:
    ...
    async def _handle_portfolio_command(self):
        ...
        await self.alert_manager.add_alert(COMMANDS_WEBHOOK,
                                           {"content": "⏳ Fetching live account portfolio value..."},
                                           "command_response")
        ...
        await self.alert_manager.add_alert(COMMANDS_WEBHOOK,
                                           {"content": f"💰 **Total Portfolio Value:** ${portfolio_value:,.2f}"},
                                           "command_response")
...
**Restart Count:** {restart_count}/{MAX_RESTART_ATTEMPTS}
                ...
                },
                {
                    ...
                    "value": """
**Trade-First Approach:** ✅ Active
**Enhanced Tick Detection:** ✅ Robinhood API Priority
**Minimum Tick Premium:** ✅ 2-Tick Enforcement
**Order Latency:** 2-4s Faster
...
# Main entry point with enhanced trade-first messaging
if __name__ == "__main__":
    while True:
        try:
            ...
            logger.info("🚀 TRADE-FIRST ENHANCEMENTS Active:")
            logger.info("   ⚡ Trade execution prioritized over all other tasks")
            logger.info("   📨 Alerts and tracking moved to background (non-blocking)")
            logger.info("   🎯 Enhanced tick size detection (Robinhood API priority)")
            logger.info("   💰 Minimum tick premium enforcement (2 ticks minimum)")
            logger.info("   📏 Proper tick size caching for performance")
            logger.info("   🔄 Position tracking and performance updates async")
            logger.info("")
            ...
            logger.info("   🎯 Market-based exit pricing with enhanced tick sizing")
